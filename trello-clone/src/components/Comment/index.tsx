...
const CommentItem: FC<Props> = ({ comment }) => {
  const { deleteCarsItemComment, changeCardsItemComment, selectedCard } =
    useTrelloContext();
  ...
  const onClickConfirmDelete = () => {
    ...
    if (isConfirmed) {
      deleteCarsItemComment(selectedCard, comment);
    }
  };

  const onClickSaveComment = () => {
    if (text) {
      changeCardsItemComment(selectedCard, { ...comment, content: text });
